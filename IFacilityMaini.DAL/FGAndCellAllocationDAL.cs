    ...
    public class FGAndCellAllocationDAL : IFGAndCellAllocation
    {
        ...
        public CommonResponse AddUpdateFgAndCellAllocation(List<AddUpdateFgCellAllocation> data)
        {
            ...
            try
            {
                ...
                {
                    var check = db.UnitworkccsTblfgandcellallocation.Where(m => m.FgAndCellAllocationId == item.fgAndCellAllocationId).FirstOrDefault();
                    if (check == null)
                    {
        ...
        public CommonResponse UploadPlanLinkageMaster(List<ReadPlanLinkage> data)
        {
            ...
            try
            {
                foreach (var item in data)
                {
                    ...
                    //if (check == null)
                    //{
                    UnitworkccsTblplanlinkagemaster tblPlanLinkageMaster = new UnitworkccsTblplanlinkagemaster();
                    tblPlanLinkageMaster.PlantId = db.UnitworkccsTblplant.Where(m => m.PlantCode == item.plantId).Select(m => m.PlantId).FirstOrDefault();
                    ...
                    db.UnitworkccsTblplanlinkagemaster.Add(tblPlanLinkageMaster);
                    db.SaveChanges();
                    obj.isStatus = true;
                    obj.response = ResourceResponse.AddedSuccessMessage;
                    //}
        ...
        public CommonResponse ReadPlanVisageMaster()
        {
            ...
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    var check = db.UnitworkccsTblplanlinkagemaster.Where(m => m.IsDeleted == 0).ToList();
                    db.UnitworkccsTblplanlinkagemaster.RemoveRange(check);
                    db.SaveChanges();
                    ...
                    foreach (var item in list1)
                    {
                        ...
                        UnitworkccsTblplanlinkagemaster tblPlanLinkageMaster = new UnitworkccsTblplanlinkagemaster();
                        tblPlanLinkageMaster.PlantId = db.UnitworkccsTblplant.Where(m => m.PlantCode == splitAry[0]).Select(m => m.PlantId).FirstOrDefault();
                        tblPlanLinkageMaster.ScheduleId = Convert.ToInt32(splitAry[1]);
                        string dt = splitAry[2];
                        if (dt.Length > 11)
                        {
                        ...
                        using (SqlConnection sqlConn = new SqlConnection(connectionString))
                        {
                            string sql = "dbo.InsertPlanVisage";
                            using (SqlCommand sqlCmd = new SqlCommand(sql, sqlConn))
                            {
                                sqlCmd.CommandType = CommandType.StoredProcedure;
                                sqlCmd.Parameters.AddWithValue("@plantId", tblPlanLinkageMaster.PlantId);
        ...
        public CommonResponse DownloadFGpartCellAllocation()
        {
            ...
            try
            {
                ...
                int StartRow = 2;
                // int slno = 1;
                var GetUtilList = db.UnitworkccsTblfgandcellallocation.Where(m => m.IsDeleted == 0).ToList();
                int slno = 1;
                if (GetUtilList.Count > 0)
                {
                    foreach (var MacRow in GetUtilList)
                    {
                        ...
                        worksheet.Cells["E" + StartRow].Value = db.UnitworkccsTblshop.Where(m => m.ShopId == MacRow.CellFinalId).Select(m => m.ShopName).FirstOrDefault();
                        worksheet.Cells["F" + StartRow].Value = db.UnitworkccsTblcell.Where(m => m.CellId == MacRow.SubCellFinalId).Select(m => m.CellName).FirstOrDefault();
        ...
        public CommonResponse UploadFgAndCellAllocation(List<UploadFgAndCellAllocation> data)
        {
            ...
            try
            {
                var check = db.UnitworkccsTblfgandcellallocation.Where(m => m.IsDeleted == 0).ToList();
                ...
                foreach (var item in data)
                {
                    ...
                    {
                        var cellFinalId = db.UnitworkccsTblshop.Where(m => m.ShopName == item.cellName).Select(m => m.ShopId).FirstOrDefault();
                        tblFgAndCellAllocation.CellFinalId = cellFinalId;
                    }

                    if (item.subcellName != null)
                    {
                        var subCellFinalId = db.UnitworkccsTblcell.Where(m => m.CellName == item.subcellName).Select(m => m.CellId).FirstOrDefault();
