...
namespace IFacilityMaini.DAL.App_Start
{
    public class CommonFunction
    {
        ...
        /// <returns></returns>
        public UnitworkccsLogintrackerdetails   GetLoginTrackerDetailsLastRow(string correctedDate, int machineId)
        {
            var loginTrackerDetails = db.UnitworkccsLogintrackerdetails.Where(m => m.CorrectedDate == correctedDate && m.MachineId == machineId && m.IsLoggedIn == 0).OrderByDescending(m => m.LoginTrackerDetailsId).FirstOrDefault();
        ...
        public UnitworkccsLogintrackerdetails GetLoginTrackerDetails(string correctedDate, int machineId, int operatorId)
        {
            var loginTrackerDetails = db.UnitworkccsLogintrackerdetails.Where(m => m.CorrectedDate == correctedDate && m.MachineId == machineId && m.IsLoggedIn == 0 && m.OperatorId == operatorId).OrderByDescending(m => m.LoginTrackerDetailsId).FirstOrDefault();
            return loginTrackerDetails;
        }
        ...
        public int ShiftDet()
        {
            ...
            var ShiftDetails = db.UnitworkccsTblshiftMstr.Where(m => m.IsDeleted == 0).ToList();
        ...
        public string ToolColorLogic(int? standardToolLife, int? actualToolLife)
        {
            ...
            try
            {
                var percentage = db.UnitworkccsTblmastervalue.Where(m => m.MasterValueName == "ToolPulseAlert").Select(m => m.MasterValue).FirstOrDefault();
