    ...
    public class FgPartNoDAL : IFgPartNo
    {
        ...
        public CommonResponse GetFgPartDetails(int machineId)
        {
            ...
            try
            {
                ...
                var noOfPartPerCycle = db.UnitworkccsTblmachinedetails.Where(m => m.MachineId == machineId).Select(m => m.NoOfPartsPerCycle).FirstOrDefault();

                var check = (from wf in db.UnitworkccsTblplanlinkagemaster
            ...
            try
            {
                ...
                var check = (from wf in db.UnitworkccsTblfgpartnodet
                             ...
                             select new
                             {
                                 ...
                                 partNo = db.UnitworkccsTblplanlinkagemaster.Where(m => m.PlanLinkageId == wf.PlanLinkageId).Select(m => m.FgPartNo).FirstOrDefault(),
                                 partDesc = db.UnitworkccsTblplanlinkagemaster.Where(m => m.PlanLinkageId == wf.PlanLinkageId).Select(m => m.PartName).FirstOrDefault(),
                                 partId = wf.PartId,
                                 shopId = db.UnitworkccsTblmachinedetails.Where(m => m.MachineId == machineId).Select(m => m.ShopId).FirstOrDefault(),
                                 cellName = db.UnitworkccsTblshop.Where(m => m.ShopId == shopId).Select(m => m.ShopName).FirstOrDefault(),
                                 cellId = db.UnitworkccsTblmachinedetails.Where(m => m.MachineId == machineId).Select(m => m.CellId).FirstOrDefault(),
                                 subCellName = db.UnitworkccsTblcell.Where(m => m.CellId == cellId).Select(m => m.CellName).FirstOrDefault(),
                                 machineName = db.UnitworkccsTblmachinedetails.Where(m => m.MachineId == machineId).Select(m => m.MachineName).FirstOrDefault(),
        ...
        public CommonResponse CloseFgPartNo(int fgPartId)
        {
            ...
            try
            {
                ...
                var check = db.UnitworkccsTblfgpartnodet.Where(m => m.FgPartId == fgPartId && m.IsDeleted == 0).FirstOrDefault();
                if (check != null)
                {
                    check.IsClosed = 1;
                ...
                if (loginTrackerDetails != null)
                {
                    var check1 = db.UnitworkccsLogintrackerdetails.Where(m => m.MachineId == check.MachineId && m.CorrectedDate == check.CorrectedDate && m.CurrentFgpart == check.FgPartId).FirstOrDefault();
                    if (check1 != null)
                    {
