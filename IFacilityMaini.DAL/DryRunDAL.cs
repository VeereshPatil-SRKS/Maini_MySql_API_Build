...
namespace IFacilityMaini.DAL
{
    public class DryRunDAL : IDryRun
    {
        ...
        public CommonResponse CloseDryRun(int dryRunId, int userId)
        {
            ...
            try
            {
                var roleId = db.UnitworkccsTbloperatordetails.Where(m => m.OpId == userId).Select(m => m.RoleId).FirstOrDefault();
                var check = db.UnitworkccsTbldryrun.Where(m => m.DryRunId == dryRunId && roleId == 11).FirstOrDefault();
                if (check != null)
                {
        ...
        public CommonResponse AutomaticCloseDryRun(int dryRunId)
        {
            ...
            try
            {
                var check = db.UnitworkccsTbldryrun.Where(m => m.DryRunId == dryRunId).FirstOrDefault();
                if (check != null)
                {
