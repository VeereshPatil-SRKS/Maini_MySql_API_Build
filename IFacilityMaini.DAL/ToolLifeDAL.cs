...
namespace IFacilityMaini.DAL
{
    public class ToolLifeDAL : IToolLife
    {
        ...
        public CommonResponse GetToolLife(string socketNumber, int machineId)
        {
            ...
            try
            {
                var check = (from m in db.UnitworkccsTbltoolandsocketdetails
                ...
                if (check != null)
                {
                    ...
                    var actualItem = db.UnitworkccsTbltoollifeoperator.Where(m => m.ToolNo == b && m.MachineId == check.machineId).OrderByDescending(m=>m.ToolLifeId).FirstOrDefault();
                    int actual = 0;
                    if (actualItem != null)
                    {
        ...
        public CommonResponse ScanToolData(string qrCode, int machineId,string fgCode)
        {
            ...
            try
            {
                ...
                if (item != null)
                {
                    ...
                    var Fgcode = db.UnitworkccsTblparts.Where(m => m.Fgcode == toolPulseIssuedPartNo.PartNo).Select(m => m.Fgcode).FirstOrDefault();
                    if (fgCode == Fgcode)
                    {
                        obj.isStatus = true;
